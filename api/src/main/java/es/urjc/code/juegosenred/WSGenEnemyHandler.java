...
public class WSGenEnemyHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if(node.get("type").asText().equals("reset"))
		{
			currentWave = 1;
		}
		else
		{
			//if(currentWave)
			//{
				int numEnemies = ThreadLocalRandom.current().nextInt((10*currentWave - 5*currentWave)+1) + 5*currentWave;
